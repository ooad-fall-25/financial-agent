...
export const StockHeader = ({ Ticker }: Props) => {
  const trpc = useTRPC();
  ...
  const {
    ...
    ...trpc.AlpacaData.fetchStockSnapshot.queryOptions({ ticker: Ticker }),
  ...
  const {
    ...
    ...trpc.AlpacaData.fetchCompanyName.queryOptions({ ticker: Ticker }),
  ...
  if (isSnapshotLoading || isNameLoading) {
    return (
  ...
  if (isSnapshotError || isNameError || !snapshotData || !companyNameData) {
    return (
  ...
  return (
    <div className="mb-6">
      <h1 className="text-2xl font-semibold text-muted-foreground">
        {companyNameData.companyName || Ticker}
      </h1>
